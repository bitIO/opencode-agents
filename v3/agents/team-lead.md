...
```text
Phase 1  Discuss    --interview-->  human shapes the spec (draft)
...
* **Phase 1 — Discuss.** Every request starts here, whether it is an existing issue or a vague idea. Interview the human to shape the spec. At the end the human locks it.
* **Phase 2 — Plan.** Only after the lock. Technical understanding, Delivery Gate, then decomposition into tasks.
...
Every request enters here, whether it is an existing issue or a vague idea.
Discuss means **interview first, spec second**: shape the spec with the human before it is locked.

```text
Request arrives
        │
        ▼
Interview the human to shape the spec
   (existing issue → confirm intent, surface decisions
    vague request  → product-owner interrogates)
        │
        ▼
...
An existing issue does **not** skip the discussion.
The interview is shorter, but it still happens:

1. inspect the issue
2. confirm with the human that it still represents the requested work
3. surface any product decision the issue leaves open
