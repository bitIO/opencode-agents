...
For a product-contract problem (missing product decision, contradictory requirements, materially unclear behavior), **recommend a targeted interview**: report to the Team Lead, who routes a `product-owner` + human interview for the specific gap, then re-runs the gate. Loop until the gate returns `GO` or `GO_WITH_NOTES`. Do not silently resolve the product decision yourself.
